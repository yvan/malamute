...
-(void) didStartReceivingResource:(MCSession *)session resourceName:(NSString *)resourceName fromPeer:(MCPeerID *)peerID withProgress:(NSProgress *)progress{
    ...
    //[_sessionWrapper sendAcknowledgementOfReceiptToPeer:peerID];
}

-(void)didReceiveData:(NSData *)data fromPeer:(MCPeerID *)peerID{
    
    NSString* newStr = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    if ([newStr isEqual:@"FIL"] ) {

        NSLog(@"%s DID RECEIVE FILE HEADER FROM PEER: %@", __PRETTY_FUNCTION__, peerID);
    }else if([newStr isEqual:@"ACK"]){
        
        NSLog(@"%s DID RECEIVE ACKNOWLEDGEMENT FROM PEER: %@", __PRETTY_FUNCTION__, peerID);
    }
    NSLog(@"%s DID RECEIVE DATA: %@, FROM PEER: %@", __PRETTY_FUNCTION__, newStr, peerID);
}
